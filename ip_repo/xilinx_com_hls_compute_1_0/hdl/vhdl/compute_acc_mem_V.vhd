...
entity compute_acc_mem_V_ram is 
    generic(
...
architecture rtl of compute_acc_mem_V_ram is 
...
begin 
    if (clk'event and clk = '1') then
        if (ce1 = '1') then 
            ...
            for i in 0 to NUM_COL - 1 loop
                if (we1(i) = '1') then
                    ram(CONV_INTEGER(addr1_tmp))((i + 1) * COL_WIDTH - 1 downto i * COL_WIDTH) := d1((i + 1) * COL_WIDTH - 1 downto i * COL_WIDTH); 
                end if;
            end loop;
        end if;
    end if;
...
architecture arch of compute_acc_mem_V is
    component compute_acc_mem_V_ram is
        port (
            clk : IN STD_LOGIC;
...
begin
    compute_acc_mem_V_ram_U :  component compute_acc_mem_V_ram
    port map (
        clk => clk,
        addr0 => address0,
        ce0 => ce0,
